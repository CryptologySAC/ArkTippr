...
class WithdrawHelp extends Help {
  async __sendReddit () {
    try {
      const message = messages.helpStickersMessage(this.platform) + messages.footer(this.platform)
  ...
  async __sendWhatsApp () {
    try {
      const message = messages.helpStickersMessage(this.platform)
