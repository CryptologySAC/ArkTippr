...
class Send {
  constructor (sender, receiver, arktoshis, mainnet, platform, receiverPlatform) {
    ...
    logger.error(`VARIABLES SEND: ${sender} ${receiver} ${arktoshis} ${platform} ${receiverPlatform} `)
  }
  ...
  async __senderHasBalance () {
    ...
    if (balance === 0 || balance + fee < this.arktoshis) {
      ...
      switch (this.platform) {
        ...
        default:
          const subject = 'ArkTippr transaction failed - Amount to low'
          await this.__sendReddit(this.sender, message, subject, this.platform)
          break
        case 'whatsapp':
          await this.__sendWhatsApp(this.sender, message)
          break
      }
