        ...
        if (mention && mention.command) {
          switch (mention.command) {
            case 'TIP':
              ...
              const tip = new Tip(this.sender, this.receiver, mention.arkToshiValue, this.mainnet, this.submissionID, this.parentID, mention.smallFooter)
            ...
            case 'REWARD':
              ...
              for (let item in mention.transfers) {
                logger.info(JSON.stringify(mention.transfers[item]))
                switch (mention.transfers[item].command) {
                  case 'TIP':
                    const tip = new Tip(this.sender, mention.transfers[item].username, mention.transfers[item].arkToshiValue, this.mainnet, this.submissionID, this.parentID, true)
                    const txId = await tip.sendReward()
                    if (txId) {
                      transactionIds.push(txId)
                    }
                    break
                  case 'STICKERS':
                    const stickersTransaction = await this.stickers(mention.transfers[item])
                    const stickersTxId = await stickersTransaction.sendMentionReply(this.submissionID)
                    if (stickersTxId) {
                      transactionIds.push(stickersTxId)
                    }
                    break
                }
              }
              const comment = await redditConfig.getComment(this.submissionID)
              const reply = transactionIds.length > 0 ? messages.rewardReply() : messages.summonedReply()
