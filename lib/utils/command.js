  ...
  async execute (body, isComment) {
    try {
      if (isComment) {
        ...
        if (mention && mention.command) {
          switch (mention.command) {
            case 'TIP':
              ...
              const tip = new Tip(this.sender, this.receiver, mention.arkToshiValue, this.mainnet, this.platform, this.platform, this.submissionID)
        ...
        const commands = await parser.parseCommand(body, this.platform)
