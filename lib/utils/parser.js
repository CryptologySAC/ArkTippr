...
const _COMMANDS = ['BALANCE', 'DEPOSIT', 'WITHDRAW', 'SEND', 'HELP', 'ADDRESS', 'STICKERS', 'TIP']
const schema = {
...
class Parser {
  ...
  **/
    ...
    try {
      ...
      switch (command) {
        ...
        case 'REWARD':
          ...
          const transfers = await this.__parseReward(parts.slice(mentionIndex + 1))
        ...
        default:
          ...
          let smallFooter = bodyParts[mentionIndex + 1] === '~'
  ...
  async __parseReward (bodyParts) {
    try {
      const platform = 'reddit'
      const requestedRewards = []
      for (let item in bodyParts) {
        const username = bodyParts[item].replace('u/', '')
        const receiver = new User(username)
        const validUser = !coinmarketcap.isCurrency(username) && parseFloat(username).toString() !== username && await receiver.isValidUser()

        let arg2 = item >= 2 ? bodyParts[item - 2] : ''
        const arg3 = item >= 2 ? bodyParts[item - 1] : ''
        if (validUser && arg3 === 'STICKERS') {
          const transfer = {
            username,
            platform,
            'command': 'STICKERS'
          ...
          if (!(parseFloat(arg2).toString() === arg2 && coinmarketcap.isCurrency(arg3)) &&
            ...
            // might be that arg3 contains both amount and currency, like 10ARK
            arg2 = ''
          }

          const arkToshiValue = await this.parseAmount(arg2, arg3)
          if (arkToshiValue !== null && arkToshiValue.arkToshiValue !== null) {
            const transfer = {
              username,
              platform,
              'command': 'TIP',
              'arkToshiValue': arkToshiValue.arkToshiValue,
              'check': arkToshiValue
            }
            requestedRewards.push(transfer)
          }
  ...
  **/
  ...
  async _parseArguments (command, bodyParts) {
    try {
      ...
      let arg4 = bodyParts[commandIndex + 4]
...
logger.info(`COMMAND PARSED: ${arg1} ${arg2} ${arg3} ${arg4}`)
      switch (command) {
