    ...
    try {
      ...
      switch (command) {
        ...
        case 'REWARD':
          ...
          return {'command': 'REWARD', transfers}
  ...
  async __parseReward (bodyParts) {
    try {
      ...
      for (let item in bodyParts) {
        const receiver = new User(bodyParts[item])
        logger.info(`Parsing reward ${bodyParts[item]}`)
        const validUser = await receiver.isValidUser()
        
        if (validUser && item >= 2) {
          logger.info(`Valid Username: ${bodyParts[item]}`)
          let arg2 = bodyParts[item -2]
          const arg3 = bodyParts[item -1]
          ...
          if (arkToshiValue !== null && arkToshiValue.arkToshiValue !== null) {
            const transfer = {
              'username': bodyParts[item].replace('u/', ''), 
