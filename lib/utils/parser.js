...
const _COMMANDS = ['BALANCE', 'DEPOSIT', 'WITHDRAW', 'SEND', 'HELP', 'ADDRESS', 'STICKERS', 'TIP', 'CURRENCIES']
const schema = {
...
class Parser {
      ...
      switch (command) {
        ...
        default:
          ...
          if (arkToshiValue !== null && arkToshiValue.arkToshiValue !== null) {
            ...
            return {command: 'TIP', arkToshiValue: arkToshiValue.arkToshiValue, check: arkToshiValue}
          }
      }
    } catch (error) {
      logger.warn(error)
  ...
  * @param {array} bodyParts The strings to parse
  **/
  async parseCommand (body, platform) {
    try {
      if (typeof (body) === 'undefined') {
        return null
      }
      ...
      for (let item in bodyParts) {
        ...
        if (_COMMANDS.includes(command)) {
          ...
          const todo = await this._checkCommand(command, argumentsBody, platform)
          if (todo !== null) {
  ...
  async _checkCommand (command, bodyParts, platform) {
    ...
    switch (command) {
      ...
      case 'WITHDRAW':
      case 'STICKERS':
        const args = await this._parseArguments(command, bodyParts, platform)
        return args
  ...
  **/
  ...
  async _parseArguments (command, bodyParts, platform) {
    try {
      ...
      switch (platform) {
        case 'reddit':
        default:
          receiver = new RedditUser(arg1)
          break
        case 'whatsapp':
          receiver = new WhatsAppUser(arg1)
          break
      }
