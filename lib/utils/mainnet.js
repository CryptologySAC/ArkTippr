  ...
  **/
  ...
  async getFee () {
    if (this.fees && this.fees.send) {
      return this.fees.send
    }

    const fees = await await network.getFromNode('/api/blocks/getFees')
    ...
    this.fees = fees.data.fees
    return fees.data.fees.send
  }
