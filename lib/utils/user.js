...
const FOOTER = '--- \n' +
'\n' +
'[How to use](https://np.reddit.com/r/arktippr/wiki/index) | [What is Ark?](https://ark.io) | [r/arktippr](https://www.reddit.com/r/arktippr)\n' +
'\n' +
...
class User {
  ...
  async getBalance (mainnet) {
    ...
    const arkAddress = await this._getAddress(this.username)
    const balance = await mainnet.getBalance(arkAddress)

    return balance
  }

  async sendBalanceReply (balance, submissionID) {
    const reply = `Your balance is ${balance} (xxx USD).\n` + FOOTER
    try {
      // submissionID = 't1_e2h1zwe'
      await redditConfig.getComment(submissionID).reply(reply)
     // submission.reply(reply)
      await redditConfig.getSubmission(submissionID).markAsRead()
      return true
    } catch (error) {
      logger.error(error.message)
      return false
  ...
  async sendTip (ark, receiver, mainnet) {
    ...
    const receiverAddress = await this._getAddress(receiver)
  ...
  async sendTipNotification (submissionID, txID, amount, usd, mainnet) {
    ...
    const reply = `u/${this.username}. You have received \`${amount} ARK ( ${usd} US$ )\`!\n` + FOOTER
    try {
