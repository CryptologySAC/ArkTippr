...
class User {
  ...
  async sendNotEnoughBalanceReply (arktoshis, usdValue) {
    const subject = 'ERROR'
    const message = messages.notEnoughBalanceMessage(arktoshis, usdValue) + messages.depositMessage(this.arkAddress) + messages.footer()
    try {
      await this.sendMessage(message, subject)
    } catch (error) {
      logger.error(error.message)
  ...
  async sendTipNotification (submissionID, transactionId, amount, usd, mainnet) {
    console.log(`Send reply to ${submissionID} with ${transactionId}`)
  ...
  async sendTransactionReply (transactionId, amount, usd, mainnet) {
    logger.info(`Send message to ${this.username} with ${transactionId}`)

    amount = mainnet.formatBalance(amount)

    const subject = 'TRANSACTION'
    const message = messages.transactionMessage(amount, usd, transactionId) + messages.footer()
  ...
  async sendDonationReply (transactionId, amount, usd, mainnet) {
    logger.info(`Send message to ${this.username} with ${transactionId}`)

    amount = mainnet.formatBalance(amount)

    const subject = 'DONATION'
    const message = messages.transactionDonationMessage() + messages.transactionMessage(amount, usd, transactionId) + messages.footer()
  ...
  async sendTransactionReceiveReply (username, transactionId, amount, usd, mainnet) {
    console.log(`Send message to ${this.username} with ${transactionId}`)

    amount = mainnet.formatBalance(amount)

    const subject = 'TRANSACTION'
    const message = messages.transactionReceiveMessage(username, amount, usd, transactionId) + messages.footer()
