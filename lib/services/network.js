...
const SERVER2 = `http://${process.env.NODE2}:${process.env.PORT}`

class Network {
  async postTransaction (transactions, server = null) { 
    if(server === null) {
      server = SERVER
    }
  ...
  async broadcast (transactions) {
    await this.postTransaction(transactions, SERVER2)
  }
  
  async getFromNode (request, params = {}) {
    logger.info(`Requesting: ${SERVER}/api/v2/${request}`)
    return axios.get(`${SERVER}/api/v2/${request}`, {
      params
    }, {
      headers: {
        'API-Version': 2
